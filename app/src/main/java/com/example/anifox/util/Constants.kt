package com.example.anifox.util

object Constants {
    const val JWT_TOKEN_KEY = "JWT_TOKEN_KEY"
    const val NAME_KEY = "NAME_KEY"
    const val EMAIL_KEY = "EMAIL_KEY"
    const val ILLEGAL_ARGUMENT_FRAGMENT_TYPE = "Invalid Fragment type"
    const val KEY_STATE = "state"
    const val KEY_TOKEN = "token"
    const val REFRESH_TOKEN = "refresh_token"
    const val APP_NAME = "AniFox"
    const val PREFERENCES_NAME = "anime_preferences"
    const val STARTING_PAGE_INDEX = 0
    const val MORE_PAGE_SIZE = 50
    const val REVIEW_PAGE = 0
    const val FAVOURITE_PAGE = 1
    const val FAVOURITE_CARDS = 50
    const val REVIEW_LIMIT = 10
    const val REVIEW_LIMIT_RANDOMIZE = 9
    const val ORDER_BY_POPULAR = "popular"
    const val SORT_BY_RATE = "popular"
    const val ORDER_BY_VIEWS = "views"
    const val STATUS_BY_FINAL = "завершён"
    const val STATUS_BY_ONGOING = "онгоинг"

    const val STATUS_LIST_Watching = "watching"
    const val STATUS_LIST_Completed = "completed"
    const val STATUS_LIST_HoldOn = "onHold"
    const val STATUS_LIST_All = "all"
    const val STYLE_BIGGER_RECYCLER = "bigger"
    const val STYLE_SMALLER_RECYCLER = "smaller"
}